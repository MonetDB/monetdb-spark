...
2. A custom MonetDB-specific [Spark Data Source]. This data source uses
   MonetDB's [`COPY BINARY INTO` feature][COPY BINARY] to upload data to
...
Place the monetdb-jdbc jar and the monetdb-spark jar on the classpath.
...
* **url**: JDBC URL to connect to. See the [MonetDB JDBC
  documentation](https://www.monetdb.org/documentation/user-guide/client-interfaces/libraries-drivers/jdbc-driver/#jdbc-connection-url-format)
  for details.
...
* **dumpdir**: if this is set, the data is written to this directory instead of
  ...
  one sql file `copy.sql` with the COPY INTO statement, plus a binary file
  for each column.

  For convenience, the top level directory will contain additional files
  `drop.sql`, `dropifexists.sql`, `create.sql`, `createifnotexists.sql`,
  `truncate.sql` and `truncatecascade.sql`.

  Can only be used in append mode.
  The **url** parameter must always be given. It is used to determine the
  exact data types used in the binary files.
